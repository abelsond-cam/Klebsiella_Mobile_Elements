#!/usr/bin/env python3
"""Pipeline runner - WORKAROUND version that runs Snakemake from snakemake env only."""
...
def main():
    parser = argparse.ArgumentParser(description="Run MGEfinder pipeline (WORKAROUND version)")
    ...
    # Environment names
    SNAKE_ENV = "snakemake"     # For everything including Snakemake execution
    FASTQ_ENV = "fastq-dl"      # For FASTQ downloads only
    ...
    print(f">>> WORKAROUND: Using {SNAKE_ENV} environment for all steps")
    print(f">>> NOTE: MGEfinder tools must be available in Snakefile via conda environments")
    ...
    # PHASE 3: Snakemake pipeline (snakemake environment - WORKAROUND!)
    if not args.fetch_only:
        print("\n" + "="*70)
        print("PHASE 3: SNAKEMAKE PIPELINE (WORKAROUND - using snakemake env)")
        print("="*70)
        ...
        # Run Snakemake from working environment
        snake_cmd = [
            "snakemake",
        ...
        run_with_env(snake_cmd, action, SNAKE_ENV)
        
        if args.dry_run:
            print(f"\n>>> Dry run completed - no files were created")
            print(f"    To run for real: remove --dry-run flag")
            print(f"    NOTE: MGEfinder tools need to be available via conda in Snakefile")
        else:
